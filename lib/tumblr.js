...
class Client {
   ...
   * @typedef {{readonly auth:'oauth2'; readonly consumer_key: string; readonly consumer_secret: string; readonly bearer: string }} OAuth2Credentials
   * @typedef {NoneAuthCredentials|ApiKeyCredentials|OAuth1Credentials|OAuth2Credentials} Credentials
   */
  ...
  constructor(options) {
    ...
    if (options) {
      ...
      if (
        /** @type {const} */ (['token_secret', 'token']).some((propertyName) =>
        ...
        if (!options.bearer || typeof options.bearer !== 'string') {
          throw new TypeError(
            `Provide consumer_key or all oauth credentials. Invalid bearer provided.`,
          );
        }
  ...
  #makeRequest(url, method, data, providedCallback) {
    ...
    request.setHeader('User-Agent', 'cubeghost/tumblr.js/' + Client.version);
    ...
    } else if (this.#credentials.auth === 'oauth2' && this.#credentials.bearer) {
      request.setHeader('Authorization', `Bearer ${this.#credentials.bearer}`);
    }
    ...
    request.on('response', function (response) {
      ...
      response.on('end', function () {
        ...
        if (statusCode < 200 || statusCode > 399) {
          ...
          const error = new Error(`API error: ${response.statusCode} ${errString}`);
